...
export default function BankAccountFormRHF({
  defaultValues,
  loading,
  onBack,
  onSubmit,
  ...
  useEffect(() => {
    ...
    if (!existing && typeof defaultValues?.cancelledChequeFile === "string") {
      setValue("cancelledChequeFile", defaultValues.cancelledChequeFile, {
  ...
  const isChequeFileRequired = !(
    watch("cancelledChequeFile") ||
    chequePreview ||
    defaultValues?.cancelledChequeFileUrl
  );
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
      <FormSection
        dense
        title="Bank Account"
        description="Enter your bank details. We'll auto-fill account holder after IFSC and account number are valid."
      >
        ...
        <FormField
          ...
          hint="Auto-fetched based on IFSC and account number."
        >
          <Input
            id="accountHolder"
