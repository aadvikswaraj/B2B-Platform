...
export default function SalesMode({ 
    ...
    const acceptOrdersAllowed = useMemo(() => {
        ...
        console.log('[SalesMode] Accept orders allowed:', result, 'for category:', category?.name);
        return result;
    }, [category]);
    ...
    const currentCommission = useMemo(() => {
        ...
        if (commissionData.mode === 'slab' && commissionData.slabs?.length > 0) {
            ...
            const sorted = [...commissionData.slabs].sort((a, b) => (a.upto || 0) - (b.upto || 0));
    ...
    useEffect(() => {
        if (!acceptOrdersAllowed && salesMode !== 'inquiry') {
            setSalesMode('inquiry');
        }
    }, [acceptOrdersAllowed, salesMode, setSalesMode]);
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {options.map((opt) => {
                        const isSelected = salesMode === opt.value;
                        const Icon = opt.icon;
                        ...
                        return (
                            <button
                                key={opt.value}
                                type="button"
                                onClick={() => !opt.disabled && setSalesMode(opt.value)}
                            ...
                            >
                                ...
                                `}>
                                    {isSelected && (
                                        <div className={`w-2.5 h-2.5 rounded-full ${opt.iconBg.replace('100', '600')}`} />
                                    )}
                                </div>
            ...
            {salesMode === 'orders' && commissionData && (
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-xl p-4 sm:p-5">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h4 className="font-semibold text-sm text-indigo-900">Platform Commission Preview</h4>
                    </div>
                    ...
                    {commissionData.mode === 'exact' ? (
                        ...
                        <div className="space-y-3">
                            ...
                            {unitPrice > 0 && currentCommission && (
                                <div className="bg-white/70 rounded-lg p-3 border border-indigo-100/50">
                                    <p className="text-xs text-gray-500 mb-2">Based on your price (₹{unitPrice.toFixed(2)}/unit)</p>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-gray-600">Commission per unit</span>
                                        <span className="font-semibold text-indigo-700">
                                            ₹{currentCommission.amount.toFixed(2)}
                                        </span>
                                    </div>
                                    <div className="flex items-center justify-between mt-1">
                                        <span className="text-sm text-gray-600">Your earnings per unit</span>
                            ...
                            <p className="text-[11px] text-indigo-600 flex items-start gap-1.5">
                                ...
                                <span>Flat commission applies to all order values in this category</span>
                            </p>
                        </div>
                        ...
                        <div className="space-y-4">
                                ...
                                <div className="sm:hidden divide-y divide-indigo-50">
                                    {[...commissionData.slabs]
                                        ...
                                        .map((slab, idx, arr) => {
                                            ...
                                            const isCurrentSlab = currentCommission?.slab?.upto === slab.upto;
                                                ...
                                                >
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <div className="text-right">
                                                            ...
                                                            <p className={`text-lg font-bold ${isCurrentSlab ? 'text-indigo-700' : 'text-gray-700'}`}>
                                                                {slab.percent}%
                                                            </p>
                                                        </div>
                                                    </div>
                                        ...
                                        {[...commissionData.slabs]
                                            ...
                                            .map((slab, idx, arr) => {
                                                ...
                                                return (
                                                    <tr 
                                                        key={idx} 
                                                        className={isCurrentSlab ? 'bg-indigo-50' : 'hover:bg-gray-50'}
                                                    >
                                                        ...
                                                        <td className={`px-4 py-3 text-right font-semibold ${isCurrentSlab ? 'text-indigo-700' : 'text-gray-700'}`}>
                                                            {slab.percent}%
                                                        </td>
                                                    </tr>
                                                );
                            ...
                            {unitPrice > 0 && currentCommission && (
                                <div className="bg-white/70 rounded-lg p-3 border border-indigo-100/50">
                                    <p className="text-xs text-gray-500 mb-2">
                                        Based on ₹{unitPrice.toFixed(2)}/unit × {moq} MOQ = ₹{(unitPrice * moq).toLocaleString()} order
                                    </p>
                                    <div className="flex items-center justify-between">
                            ...
                            <p className="text-[11px] text-indigo-600 flex items-start gap-1.5">
                                ...
                                <span>Higher order values attract lower commission rates. Encourage bulk orders!</span>
                            </p>
                        </div>
                    ) : null}
                </div>
