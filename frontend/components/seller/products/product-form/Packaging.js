...
const PackagingLevelRow = ({ index, register, watch, remove, getTotalUnits, isLast, onShippingChange, isLocked }) => {
    const isBase = index === 0;
    const levelData = watch(`packagingLevels.${index}`);
    const isShipping = levelData?.isShippingUnit;
    const prevLevelName = index > 0 ? (watch(`packagingLevels.${index - 1}.name`) || `L${index}`) : "";
    
    // Auto-calc weight hint
    const prevWeight = index > 0 ? (parseFloat(watch(`packagingLevels.${index - 1}.weight`)) || 0) : 0;
    const qty = parseFloat(levelData?.containsQty) || 0;
    const suggestedWeight = index > 0 && prevWeight && qty ? (prevWeight * qty).toFixed(2) : "";
            ...
            `}>
                ...
                <div className="p-4 pt-6 md:p-5 grid grid-cols-1 md:grid-cols-12 gap-y-4 gap-x-6 items-start">
                    ...
                    <div className="md:col-span-3">
                        ...
                        <Input
                            {...register(`packagingLevels.${index}.name`, { required: "Name is required" })}
                    ...
                    <div className="md:col-span-3">
                        ...
                        {isBase ? (
                            ...
                            <div className="flex shadow-sm rounded-md ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-blue-600">
                                <input
                                    type="number"
                                    {...register(`packagingLevels.${index}.containsQty`, { required: true, min: 1 })}
                                ...
                                <div className="flex items-center px-3 bg-gray-50 border-l border-gray-200 rounded-r-md">
                                    ...
                                    <span className="text-xs font-medium text-gray-700 max-w-[60px] truncate" title={prevLevelName}>
                    ...
                    <div className="md:col-span-2">
                        ...
                        <div className="relative rounded-md shadow-sm">
                            <input
                                type="number"
                                step="0.001"
                                {...register(`packagingLevels.${index}.weight`, { required: "Weight is required" })}
                    ...
                    <div className="md:col-span-4">
                        <label className="block text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1">
                            Dimensions <span className="normal-case font-normal text-gray-400">(L × W × H cm)</span>
                        </label>
                        <div className="grid grid-cols-3 gap-2">
                            <input 
                                type="number" 
                                {...register(`packagingLevels.${index}.dimensions.l`, { required: true })} 
                                placeholder="L" 
                            ...
                            <input 
                                type="number" 
                                {...register(`packagingLevels.${index}.dimensions.w`, { required: true })} 
                                placeholder="W" 
                                className="block w-full rounded-md border-0 py-1.5 text-center text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-xs" 
                            />
                            <input 
                                type="number" 
                                {...register(`packagingLevels.${index}.dimensions.h`, { required: true })} 
                                placeholder="H" 
                                className="block w-full rounded-md border-0 py-1.5 text-center text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-xs" 
                            />
                        </div>
                    ...
                    <div className="md:col-span-12 flex items-center justify-end gap-4 pt-2 border-t border-gray-50 mt-1">
                         ...
                         <label className="flex items-center gap-2 cursor-pointer group">
                            <span className="text-xs text-gray-500 group-hover:text-gray-700 transition-colors select-none">Stackable?</span>
                        ...
                        <label className={`flex items-center gap-2 cursor-pointer px-3 py-1.5 rounded-full transition-colors border ${isShipping ? "bg-blue-50 border-blue-200" : "bg-gray-50 border-transparent hover:bg-gray-100"} ${isLocked ? "bg-gray-100 border-gray-200 cursor-not-allowed opacity-75" : ""}`}>
                             <input 
                                    type="checkbox" 
                                    {...register(`packagingLevels.${index}.isShippingUnit`, {
                                        onChange: (e) => onShippingChange && onShippingChange(index, e.target.checked)
                                    })} 
                                    disabled={isLocked}
                             ...
                             />
                             <span className={`text-xs select-none font-bold uppercase tracking-wide ${isShipping ? "text-blue-700" : "text-gray-500"} ${isLocked ? "text-gray-400" : ""}`}>
...
export default function Packaging({ control, register, errors, watch, setValue }) {
  ...
  const onShippingChange = (index, isChecked) => {
    if (isChecked) {
        ...
        for (let i = index + 1; i < fields.length; i++) {
            setValue(`packagingLevels.${i}.isShippingUnit`, true, { shouldDirty: true, shouldTouch: true });
        }
    } 
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
      ...
      <div className="lg:col-span-3">
          ...
          <div className="hidden md:flex gap-4 px-0 mb-2">
              <div className="w-8 shrink-0"></div> {/* Spacer for alignment with Badge Rail */}
              <div className="flex-1 grid grid-cols-12 gap-6 px-5 text-xs font-bold text-gray-400 uppercase tracking-wider">
                  ...
                  <div className="col-span-2">Weight <span className="normal-case font-normal">(kg)</span></div>
                  <div className="col-span-4 text-center">Dimensions <span className="normal-case font-normal">(cm)</span></div>
              </div>
          </div>
          
          <div className="space-y-1">
              {fields.map((field, index) => {
                 const prevIsShipping = index > 0 ? (levels?.[index - 1]?.isShippingUnit || false) : false;
                 return (
                 <PackagingLevelRow 
                    key={field.id}
                    index={index}
          ...
          <div className="mt-4 flex gap-4">
              <div className="w-8 shrink-0"></div> {/* Spacer for alignment with rail */}
              <button
                type="button"
                onClick={() => {
                    const lastLvl = levels && levels.length > 0 ? levels[levels.length - 1] : null;
                    ...
                    return (
                        <div key={i} className="relative pl-4 py-1.5 text-xs">
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    <div className={`font-medium ${lvl.isShippingUnit ? "text-blue-700" : "text-gray-700"}`}>
                                        {name}
                                    </div>
                                    <div className="text-[10px] text-gray-400">
                                        {i === 0 ? "Base" : `x${lvl.containsQty || 1}`} ({totalUnits})
                                    </div>
                                </div>
                                {lvl.weight && (
