 ...
 */
export function Spinner({ size = 'md', variant = 'primary', className, label = 'Loading...', srOnly = true }) {
  const sizeMap = {
  ...
  return (
    ...
    >
      ...
      />
      {label && (
        <span className={clsx('ml-2 text-sm font-medium', srOnly && 'sr-only')}>{label}</span>
      )}
    </span>
  );
}
...
export function ScreenSpinner({ visible, backdrop = 'light', text = 'Loading...' }) {
  ...
  return (
    <div
      ...
      aria-live="assertive"
    >
      <Spinner size="xl" variant={dark ? 'inverse' : 'primary'} srOnly={false} label={text} className="drop-shadow" />
