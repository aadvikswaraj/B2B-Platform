...
export default function FileInput({
  ...
  const isImage = useMemo(() => {
    ...
    if (effectiveUrl)
      return (
        !/\.pdf($|\?|#)/i.test(effectiveUrl) &&
        !/^data:application\/pdf/i.test(effectiveUrl)
      );
    return false;
  ...
  const maxBytes =
    typeof maxSizeBytes === "number" ? maxSizeBytes : maxSizeMB * 1024 * 1024;
        ...
        {!currentFile && !effectiveUrl ? (
          ...
          >
            <span className="truncate text-gray-400">
              {placeholder || `Choose file (images/PDF, up to ${maxSizeMB}MB)`}
            </span>
          ...
          >
            <div className="min-w-0 flex-1">
              <p className="truncate text-gray-700">
                {currentFile?.name || (effectiveUrl ? "File uploaded" : "Selected file")}
              </p>
              {currentFile ? (
              ...
              {effectiveUrl && !currentFile && (
                <a
                  href={effectiveUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 mt-1 text-xs text-green-700 bg-green-50 border border-green-200 rounded px-2 py-0.5 font-medium hover:underline"
                >
                  <svg className="h-3 w-3 text-green-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  File uploaded
                </a>
              )}
            </div>
