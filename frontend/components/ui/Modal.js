...
export default function Modal({
  ...
  const handleTouchStart = (e) => {
    ...
    const scrollTop =
      contentRef.current?.querySelector(".modal-content")?.scrollTop || 0;
  ...
  const modalContent = (
    <div
      className={clsx(
        ...
        "sm:rounded-xl sm:w-full sm:max-h-[80vh] sm:translate-y-0", // Added max-h constraint
        !center && "sm:mt-20",
        {
          sm: "sm:max-w-sm",
          md: "sm:max-w-lg",
        ...
        mobileMode === "fullscreen" && [
          "fixed inset-0 w-full h-full rounded-none sm:relative sm:inset-auto sm:h-auto",
        ],
        mobileMode === "popup" && ["w-full max-w-lg rounded-xl mx-4 mt-20"],
      )}
      style={
        mobileMode === "drawer" ? { transform: `translateY(${dragY}px)` } : {}
      }
      ref={contentRef}
    ...
    >
      ...
      {showHeader ? (
        <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-gray-100 shrink-0">
          ...
          {showCloseButton && (
            <button
              onClick={onClose}
              className="rounded-md p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
              <span className="sr-only">Close</span>
              <svg viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
                <path
                  fillRule="evenodd"
                  d="M10 8.586 4.95 3.536 3.536 4.95 8.586 10l-5.05 5.05 1.414 1.414L10 11.414l5.05 5.05 1.414-1.414L11.414 10l5.05-5.05-1.414-1.414L10 8.586Z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
      ...
      <div
        className={clsx(
          ...
          // Mobile fullscreen: flex-1 to fill height
          mobileMode === "fullscreen"
            ? "flex-1 sm:flex-1 sm:min-h-0"
            : "flex-1 min-h-0", // flexible height with scroll for both desktop and mobile popup
        )}
      >
        {children}
      </div>
    ...
    <Fragment>
      ...
      <div
        className={clsx(
          ...
          mobileMode === "drawer"
            ? "items-end sm:justify-center"
            : "items-center justify-center",
          center ? "sm:items-center" : "sm:items-start",
          // Padding
          mobileMode === "fullscreen" || mobileMode === "drawer"
            ? "p-0 sm:p-6"
            : "p-4 sm:p-6",
        )}
      >
