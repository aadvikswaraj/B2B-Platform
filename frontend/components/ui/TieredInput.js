...
import { PlusIcon, TrashIcon, InformationCircleIcon, ExclamationCircleIcon, ArrowsUpDownIcon } from "@heroicons/react/24/outline";
...
export default function TieredInput({
  ...
  if (mode !== 'single') {
    ...
    if (required && minRows > 0 && value.length < minRows) {
      globalErrors.push(`At least ${minRows} row${minRows > 1 ? 's' : ''} required`);
    }
    
    if (validate) {
      const result = typeof validate === 'function' ? validate(value) : validate;
      if (Array.isArray(result)) {
       ...
       value.forEach((s, i) => {
          ...
          columns.forEach(col => {
             ...
             // 2. Numeric check
             if (col.type === 'number' && cellValue !== "" && cellValue !== null && cellValue !== undefined) {
          ...
          if (orderBy && !currentErrors[orderBy]) {
             if (s[orderBy] !== "" && s[orderBy] !== null && s[orderBy] !== undefined) {
  ...
  const nonEmptySlabs = value.filter(s => Object.values(s).some(v => v !== "" && v !== null && v !== undefined));
        ...
        <div className="flex flex-col lg:flex-row">
                            ...
                            <div className={clsx(
                                ...
                                {columns.map((col) => (
                                    <div key={col.key} className="flex-1 relative">
                                        <span className="sm:hidden text-xs font-medium text-gray-500 mb-1 block">{col.label}</span>
                                        <div className="relative">
                                            {col.prefix && (
                                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                                    <span className="text-gray-500 sm:text-sm">{col.prefix}</span>
                                                </div>
                                            )}
                                            <input
                                                type={col.type || "text"}
                                                value={slab[col.key]}
                                                onChange={(e) => updateSlab(index, col.key, e.target.value)}
                                                placeholder={col.placeholder}
                                                className={clsx(
                                                    "block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-1.5",
                                                    col.prefix && "pl-7",
                                                    col.suffix && "pr-8",
                                                    rowError?.[col.key] && "border-red-300 focus:border-red-500 focus:ring-red-500"
                                                )}
                                                min={col.min}
                                            />
                                            {col.suffix && (
                                                <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                                    <span className="text-gray-500 sm:text-sm">{col.suffix}</span>
                                                </div>
                                            )}
                                        </div>
                                        {/* Inline Cell Error */}
                                        ...
                                        {/* Mobile Error */}
                                        {rowError?.[col.key] && (
                                              <p className="text-xs text-red-600 mt-1 sm:hidden">{rowError[col.key]}</p>
                                        )}
                                    </div>
                                ))}
            ...
            <div className="lg:w-72 bg-gray-50/50 lg:bg-transparent border-t lg:border-t-0 lg:border-l border-gray-100">
                ...
                <span className="lg:hidden">
                    {mobilePreviewOpen ? <ChevronUpIcon className="h-4 w-4" /> : <ChevronDownIcon className="h-4 w-4" />}
                </span>
                </button>
                ...
                mobilePreviewOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0 lg:max-h-full lg:opacity-100"
                )}>
