...
export default function Sidebar({
  ...
  return (
    ...
    >
      ...
      {/* List */}
      <div
        className={`${loading ? "flex items-center justify-center" : ""} flex-1 overflow-y-auto custom-scrollbar`}
      >
        {loading ? (
          <Spinner size="lg" />
        ) : contacts?.length > 0 ? (
          <div className="divide-y divide-gray-50">
            {contacts.map((contact) => (
              <ConversationItem
                key={contact._id}
                contact={contact}
                isActive={selectedChatId === contact._id}
          ...
          <div className="flex flex-col items-center justify-center h-48 text-gray-400 px-6 text-center">
            ...
            <p className="text-sm">No contacts found matching your creteria.</p>
          </div>
        )}
      </div>
    </div>
