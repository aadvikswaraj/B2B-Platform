...
export default function AdminProductsPage() {
  ...
  const handleDecision = async () => {
    ...
    try {
      let res;
      if (action === 'approve') {
        res = await ProductVerificationAPI.approve(product._id);
      } else if (action === 'reject') {
        res = await ProductVerificationAPI.reject(product._id, decisionReason);
      }
  ...
  const productColumns = [
      ...
      render: (p) => (
        <div className="flex items-center gap-3 max-w-[220px]">
          ...
          <div className="flex flex-col">
            <span className="font-medium leading-tight line-clamp-1" title={p.title || p.name}>
              {p.title || p.name}
            </span>
    ...
    },
    {
      ...
      render: (p) => (
        <div>
          <div className="text-sm text-gray-900">{p.seller?.name || p.user?.name}</div>
          <div className="text-xs text-gray-500">{p.seller?.email || p.user?.email}</div>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      header: 'Status',
      render: (p) => (
        <span className={`px-2 inline-flex text-xs font-semibold rounded-full capitalize ${
          (p.moderationStatus || p.status) === 'approved' || (p.moderationStatus || p.status) === 'active' 
      ...
      {decisionModal && (
        ...
        >
          <div className="space-y-4">
            <p>
              Are you sure you want to {decisionModal.action} <b>{decisionModal.product.title || decisionModal.product.name}</b>?
            </p>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <Button 
                ...
                disabled={decisionModal.action === 'reject' && !decisionReason.trim()}
              >
                Confirm
              </Button>
            </div>
