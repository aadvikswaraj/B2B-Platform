        ...
        render: (row) => {
          const priceValue =
            typeof row.price === "object"
              ? row.price?.singlePrice || row.price?.slabs?.[0]?.price || "—"
              : row.price;
          return (
            <div className="text-gray-900">
              {priceValue} {row.currency}
            </div>
          );
        },
      },
            ...
            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
              row.moderationStatus === "approved"
                ...
                : row.moderationStatus === "rejected"
                  ...
                  : row.moderationStatus === "pending_update"
        ...
        render: (row) => (
          <div className="flex gap-2">
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                router.push(`/admin/product-verification/${row._id}`);
              }}
            >
              Review
            </Button>
  ...
  return (
    <div className="mt-5">
      <ManagementPanel
        ...
        getRowId={(r) => r._id}
        itemLink={(r) =>
          r._id ? `/admin/product-verification/${r._id}` : null
        }
        {...handlers}
