...
export default function AddProduct() {
  ...
  // Sales Mode State
  const [isOrder, setIsOrder] = useState(true); // true: orders, false: inquiry
  ...
  const {
    ...
    defaultValues: {
      ...
      shortDesc: "",
      brandId: "",
      productImages: "", // Virtual field for image validation
      specs: [],
      attributes: {},
      moq: 1,
      stock: 0,
  ...
  // Sync images state to RHF for validation
  useEffect(() => {
    if (images.length > 0) {
      setImageError(null);
      setValue("productImages", images.join(","), { shouldValidate: true });
    } else {
      setValue("productImages", "", { shouldValidate: true });
    }
  }, [images, setValue]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log(
          "Accept Orders Allowed:",
          resolveAcceptOrders(fullCategory),
        );
      } catch (e) {
        console.error(e);
  ...
  const onSubmit = async (data) => {
    ...
    // Process Images validation is handled by RHF via productImages field
    if (images.length === 0) {
    ...
    const payload = {
      ...
      priceType,
      singlePrice:
        priceType === "single" ? parseFloat(data.price) || 0 : undefined,
      priceSlabs: priceType === "slab" ? data.priceSlabs : undefined,
      moq: parseInt(data.moq) || 1,
      stock: isOrder ? parseInt(data.stock) || 0 : undefined,
      ...
      ...(isOrder && {
        ...
        dispatchTimeParcel: {
          ...
          days:
            parcelMode === "single"
              ? parseInt(data.dispatchTimeParcel) || 0
              : undefined,
          slabs:
            parcelMode === "slab" ? data.dispatchTimeParcelSlabs : undefined,
        },
        ...
        dispatchTimeFreight: {
          ...
          days:
            freightMode === "single"
              ? parseInt(data.dispatchTimeFreight) || 0
              : undefined,
          slabs:
            freightMode === "slab" ? data.dispatchTimeFreightSlabs : undefined,
        },
      ...
      packagingDetails:
        isOrder ? data.packagingDetails : undefined,
  ...
  const tabs = [
    {
      ...
      disabled: false,
      // Tab Content 0: Category Selection
      content: (
            ...
            <div className="border rounded-lg p-4 bg-gray-50 min-h-[300px]">
              ...
              {category && (
                <div className="mt-4 p-3 bg-green-50 text-green-700 rounded-md border border-green-200 flex items-center justify-between">
                  <span>
                    Selected: <strong>{category.name}</strong>
                  </span>
    ...
    },
    {
      ...
      disabled: !category,
      // Tab Content 1: The Full Form
      content: (
        <div className="space-y-8 animate-in fade-in duration-300">
          {/* 1. Basic Identity */}
          <FormSection
            title="Basic Identity"
            description="Commercial details and branding."
            icon={TagIcon}
          >
            <BasicIdentity
              control={control}
          ...
          <FormSection
            ...
            description="Product images, videos and datasheets."
            icon={PhotoIcon}
          >
            <MediaDocs
              images={images}
              ...
              error={imageError || errors.productImages?.message}
              register={register}
            />
          </FormSection>
          ...
          <FormSection
            ...
            description="Material, finishes, and industry attributes."
            icon={CubeIcon}
          >
            <TechSpecs
              register={register}
          ...
          {isOrder && (
            <FormSection
              title="Packaging & Physical Structure"
              description="How this product is grouped, stored, and shipped."
              icon={CubeIcon}
            >
              <Packaging
                control={control}
          ...
          {isOrder && (
            <FormSection
              ...
              description="Processing times for Parcel vs Freight modes."
              icon={TruckIcon}
            >
              <DispatchLogistics
                register={register}
          ...
          {isOrder && (
            <FormSection
              title="Cost Support & Subsidies"
              description="Offer free shipping or payment fee absorption to boost sales."
              icon={BanknotesIcon}
            >
              <CostSupport
                register={register}
          ...
          {isOrder && (
            <div className="pt-4">
              <ProfitabilitySimulator watch={watch} category={category} />
            </div>
          )}
