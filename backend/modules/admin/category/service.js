import { Category, File } from "../../../models/model.js";
import { cleanupOldFile } from "../../../utils/fileCleanup.js";

export const listCategories = async (query, skip, limit, sort = { createdAt: -1 }) => {
...
export const getAncestors = async (ids) => {
    return await Category.find({ _id: { $in: ids } }).select("commission").lean();
};
export const updateBulkStatus = async (ids, isActive) => {
    return await Category.updateMany({ _id: { $in: ids } }, { $set: { isActive } });
};
...
export const updateCategory = async (id, data, originalImageId = null) => {
    ...
    const dbImageId = currentCategory?.image?.toString?.() || currentCategory?.image;
