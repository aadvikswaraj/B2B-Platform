...
export const create = async (req, res) => {
  try {
    ...
    res.locals.response = {
      success: false,
      message:
        error.code === 11000 ? "Role name already exists" : error.message,
      ...
      if (strategy === "reassign") {
        ...
        if (String(targetRoleId) === String(id)) {
          res.locals.response = {
            success: false,
            message:
              "targetRoleId cannot be the same as the role being deleted",
            status: 400,
          };
          return sendResponse(res);
        }
        ...
        res.locals.response = {
          success: false,
          message:
            "Invalid strategy. Use 'reassign', 'delete-users', or 'none'",
          status: 400,
        };
        return sendResponse(res);
      }
